...
func genMapping(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		for i := range binding.argNames {
			...
			case "*github.com/mkirov/delve/service/api.LoadConfig":
			...
			case "*github.com/mkirov/delve/service/api.EvalScope":
...
func genDocs(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		fmt.Fprintf(&buf, "%s(%s) | Equivalent to API call [%s](https://godoc.org/github.com/mkirov/delve/service/rpc2#RPCServer.%s)\n", binding.name, argNames, binding.fn.Name(), binding.fn.Name())
	}

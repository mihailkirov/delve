...
func setup() {
	pkgs, err := packages.Load(&packages.Config{Mode: packages.LoadSyntax, Fset: fset}, "github.com/mkirov/delve/pkg/proc")
	if err != nil {
...
func isProcVariableDecl(stmt ast.Stmt, tinfo *types.Info) *ast.Ident {
	...
	if typ == nil || typ.String() != "*github.com/mkirov/delve/pkg/proc.Variable" {
		return nil
	}
	return ident
}
